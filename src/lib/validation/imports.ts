...
export const tiktokImportRequestSchema = z.object({
  jobName: optionalTrimmedString,
  country: z.preprocess((value) => emptyToUndefined(value) ?? "com", z.string().trim().min(2).max(10)),
  query: z.preprocess(emptyToUndefined, z.string().trim().min(1, "请填写 TikTok Shop 搜索关键词")),
  localCategoryId: z.preprocess(emptyToUndefined, z.string().trim().min(1, "请选择本地分类")),
  targetCount: z.preprocess((value) => {
    const normalized = emptyToUndefined(value);
    if (normalized === undefined) return 10;
  ...
  candidatePoolSize: z.preprocess((value) => {
    const normalized = emptyToUndefined(value);
    if (normalized === undefined) return 20;
    if (typeof normalized === "number") return normalized;
    if (typeof normalized === "string") return Number(normalized);
    return normalized;
  }, z.number().int().min(1).max(30)),
  ...
  minRating: optionalNumber.refine((value) => value == null || (value >= 0 && value <= 5), "评分范围必须在 0~5 之间"),
  minReviews: optionalInt.refine((value) => value == null || value >= 0, "评论数不能小于 0"),
  minSold: optionalInt.refine((value) => value == null || value >= 0, "销量不能小于 0"),
  onlyOfficialShop: booleanLike.default(false),
  autoPublish: booleanLike.default(false),
  downloadImages: booleanLike.default(true),
  featuredTopN: z.preprocess((value) => {
    const normalized = emptyToUndefined(value);
...
export const tiktokImportCronRequestSchema = z.object({
  jobs: z.array(tiktokImportRequestSchema).min(1).max(20),
});
...
export type TikTokImportRequest = z.infer<typeof tiktokImportRequestSchema>;
export type TikTokImportCronRequest = z.infer<typeof tiktokImportCronRequestSchema>;
